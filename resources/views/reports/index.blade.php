...
@section('content')
    <div class="flex flex-col min-h-screen bg-[#07090e] text-slate-100 font-sans">
        ...
        <!-- Top Header Bar -->
        <header
            class="h-16 bg-[#0f131f]/95 border-b border-slate-800/80 px-4 sm:px-8 flex items-center justify-between z-30 shrink-0 backdrop-blur-md">
            <div class="flex items-center gap-4">
                <a href="{{ route('dashboard') }}"
                    class="flex items-center justify-center w-10 h-10 rounded-xl bg-slate-800/80 hover:bg-slate-700 text-slate-300 hover:text-white transition-all border border-slate-700/50">
            ...
            <div class="flex items-center gap-3 no-print">
                <button onclick="openZReportModal()"
                    class="px-5 py-2.5 rounded-2xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white text-xs font-black transition flex items-center gap-2 shadow-lg shadow-emerald-600/30 border border-emerald-500/30 cursor-pointer">
        ...
        <div class="p-4 sm:p-8 space-y-8 flex-1">
            ...
            <div
                class="p-4 rounded-3xl bg-[#111524] border border-slate-800/80 shadow-xl flex flex-col md:flex-row items-center justify-between gap-4 no-print">
                ...
                <form method="GET" action="{{ route('reports.index') }}" class="flex items-center gap-2 w-full md:w-auto">
                    ...
                    <div class="flex items-center gap-2 bg-[#161b2e] border border-slate-800 p-1.5 rounded-2xl text-xs">
                        ...
                        <input type="date" name="end_date" value="{{ request('end_date', $endDate->format('Y-m-d')) }}"
                            class="bg-slate-900 border border-slate-800 text-white px-2.5 py-1.5 rounded-xl outline-none focus:border-fuchsia-500">
                        <button type="submit"
                            class="px-3 py-1.5 rounded-xl bg-fuchsia-600 hover:bg-fuchsia-500 text-white font-bold transition shadow cursor-pointer">
                            Filtrele
                        </button>
                    </div>
                </form>
            ...
            <div class="text-xs font-semibold text-slate-400 flex items-center justify-between">
                <span>Rapor Dönemi: <strong class="text-fuchsia-400 font-mono">{{ $startDate->format('d.m.Y H:i') }} -
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                ...
                <!-- TOPLAM NET CİRO -->
                <div
                    class="p-6 rounded-3xl bg-gradient-to-br from-[#13192e] to-[#18112e] border border-emerald-500/30 shadow-2xl relative overflow-hidden">
                    ...
                    <div
                        class="mt-3 flex items-center justify-between text-xs text-slate-400 border-t border-slate-800/80 pt-3">
                        <span>Adisyon Sayısı: <strong
                                class="text-white font-mono">{{ $stats['total_checks_count'] }}</strong></span>
                        <span>Ortalama: <strong
                                class="text-white font-mono">₺{{ number_format($stats['avg_check_amount'], 2) }}</strong></span>
                    </div>
                </div>

                <!-- KASA ÖDEME DAĞILIMI (Z-RAPORU KASA ÖZETİ) -->
                <div
                    class="p-6 rounded-3xl bg-[#111524] border border-slate-800/80 shadow-2xl col-span-1 md:col-span-2 flex flex-col justify-between">
                    <div class="flex items-center justify-between mb-2">
                        ...
                        <span class="text-xs font-mono font-bold text-slate-300">
                            Toplam Tahsilat: <strong
                                class="text-emerald-400">₺{{ number_format($paymentBreakdown['total'], 2) }}</strong>
                        </span>
                    </div>
                    ...
                    <div class="grid grid-cols-3 gap-3 my-auto">
                        ...
                        <div class="p-3 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 text-center">
                            <span
                                class="text-[10px] font-bold text-emerald-300 uppercase block flex items-center justify-center gap-1"><i
                                    class="fi fi-rr-money-bill-wave text-xs"></i> NAKİT</span>
                        ...
                        <div class="p-3 rounded-2xl bg-indigo-500/10 border border-indigo-500/20 text-center">
                            <span
                                class="text-[10px] font-bold text-indigo-300 uppercase block flex items-center justify-center gap-1"><i
                                    class="fi fi-rr-credit-card text-xs"></i> KREDİ KARTI</span>
                        ...
                        <div class="p-3 rounded-2xl bg-amber-500/10 border border-amber-500/20 text-center">
                            <span
                                class="text-[10px] font-bold text-amber-300 uppercase block flex items-center justify-center gap-1"><i
                                    class="fi fi-rr-ticket text-xs"></i> YEMEK KARTI</span>
                ...
                <div
                    class="p-6 rounded-3xl bg-[#111524] border border-slate-800/80 shadow-2xl flex flex-col justify-between space-y-3">
                    <span class="text-xs font-bold text-slate-400 uppercase tracking-wider block">İndirim & İptal
                        Özeti</span>

                    <div class="space-y-2 text-xs">
                        <div
                            class="flex justify-between items-center p-2 rounded-xl bg-slate-900/60 border border-slate-800">
                            <span class="text-slate-400">İskonto / İndirim:</span>
                            <span
                                class="font-mono font-bold text-amber-400">₺{{ number_format($stats['total_discounts'], 2) }}</span>
                        </div>
                        <div
                            class="flex justify-between items-center p-2 rounded-xl bg-slate-900/60 border border-slate-800">
                            <span class="text-slate-400">İkram Edilen ({{ $stats['complimentary_count'] }} Adet):</span>
                            <span
                                class="font-mono font-bold text-indigo-400">₺{{ number_format($stats['complimentary_total_amount'], 2) }}</span>
                        </div>
                        <div
                            class="flex justify-between items-center p-2 rounded-xl bg-rose-500/10 border border-rose-500/20">
                            <span class="text-rose-300 font-semibold">İptal/Zayi Kaybı
                                ({{ $stats['cancelled_items_count'] }} Adet):</span>
                            <span
                                class="font-mono font-bold text-rose-400">₺{{ number_format($stats['cancelled_loss_amount'], 2) }}</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-[#111524] border border-slate-800/80 rounded-3xl overflow-hidden shadow-2xl space-y-2">
                <div
                    class="p-5 border-b border-slate-800/80 bg-[#161b2e] flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    <div>
                        <h3 class="text-base font-extrabold text-white flex items-center gap-2">
                            <i class="fi fi-rr-receipt text-emerald-400 text-lg"></i>
                            Tüm Adisyonlar & Tekil Sipariş Geçmişi
                        </h3>
                        <p class="text-xs text-slate-400 mt-0.5">Seçilen dönemdeki her adisyonun saat kaçta açıldığı, hangi
                            masada olduğu, hangi ürünleri içerdiği ve ne ile ödendiği</p>
                    </div>
                </div>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-left text-xs text-slate-300">
                        <thead
                            class="bg-[#161b2e] text-slate-400 uppercase tracking-wider text-[10px] font-extrabold border-b border-slate-800">
                            <tr>
                            ...
                            @forelse($checksHistory as $checkItem)
                                ...
                                <tr class="hover:bg-slate-900/40 transition">
                                    <td class="py-4 px-6 font-mono text-slate-300">
                                        <span
                                            class="text-sm font-black text-white block">{{ $checkItem->opened_at ? $checkItem->opened_at->format('H:i') : '--:--' }}</span>
                                        <span
                                            class="text-[10px] text-slate-500">{{ $checkItem->opened_at ? $checkItem->opened_at->format('d.m.Y') : '' }}</span>
                                    </td>
                                    ...
                                    <td class="py-4 px-6">
                                        @if($checkItem->diningTable)
                                            <span
                                                class="px-2.5 py-1 rounded-lg bg-indigo-500/10 text-indigo-300 font-bold border border-indigo-500/20">
                                                {{ $checkItem->diningTable->name }}
                                                @if($checkItem->diningTable->hall)
                                                    <span
                                                        class="text-[10px] text-slate-400">({{ $checkItem->diningTable->hall->name }})</span>
                                                @endif
                                            </span>
                                        @else
                                            <span
                                                class="px-2.5 py-1 rounded-lg bg-amber-500/10 text-amber-300 font-bold border border-amber-500/20 inline-flex items-center gap-1">
                                    ...
                                    <td class="py-4 px-6 max-w-xs">
                                        <div class="flex flex-wrap gap-1">
                                            @foreach($checkItem->items as $it)
                                                <span
                                                    class="px-2 py-0.5 rounded bg-slate-800 text-slate-200 text-[11px] font-semibold border border-slate-700/50">
                                                    {{ number_format($it->quantity, 0) }}x
                                                    {{ !empty($it->product_name) ? $it->product_name : ($it->product?->name ?: 'Özel Sipariş') }}
                                                </span>
                                            @endforeach
                                        </div>
                                    </td>
                                    ...
                                    <td class="py-4 px-6 text-center">
                                        @if(in_array('nakit', $paymentTypes))
                                            <span
                                                class="px-2.5 py-1 rounded-lg bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 text-[11px] font-bold inline-flex items-center gap-1">
                                                <i class="fi fi-rr-money-bill-wave text-xs"></i> Nakit
                                            </span>
                                        @elseif(in_array('kredi_karti', $paymentTypes))
                                            <span
                                                class="px-2.5 py-1 rounded-lg bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 text-[11px] font-bold inline-flex items-center gap-1">
                                                <i class="fi fi-rr-credit-card text-xs"></i> K. Kartı
                                            </span>
                                        @elseif(in_array('yemek_karti', $paymentTypes))
                                            <span
                                                class="px-2.5 py-1 rounded-lg bg-amber-500/10 text-amber-400 border border-amber-500/20 text-[11px] font-bold inline-flex items-center gap-1">
                                    ...
                                    <td class="py-4 px-6 text-right font-mono font-black text-emerald-400 text-base">
                                        ...
                                        @if($checkItem->discount_total > 0)
                                            <span
                                                class="block text-[10px] text-amber-400 font-normal">(-₺{{ number_format($checkItem->discount_total, 2) }}
                                                İsk.)</span>
                                        @endif
                                    </td>
                                </tr>
            ...
            <div class="p-6 rounded-3xl bg-[#111524] border border-slate-800/80 shadow-2xl space-y-4">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-xs text-slate-400 mt-0.5">Günün hangi saatlerinde ciro ve adisyon yoğunlaşmasını
                            gösterir</p>
                    </div>
                </div>
                ...
                <div
                    class="grid grid-cols-12 md:grid-cols-24 gap-1.5 items-end h-40 pt-6 px-2 bg-[#0c0e17] rounded-2xl border border-slate-800/80 overflow-x-auto custom-scrollbar">
                    @foreach($hourlyData as $item)
                        ...
                        <div class="flex flex-col items-center gap-1 group relative h-full justify-end min-w-[28px]">
                            ...
                            <div
                                class="absolute -top-12 hidden group-hover:flex flex-col items-center bg-slate-900 border border-slate-700 px-2 py-1 rounded-lg text-[10px] font-mono z-30 shadow-xl whitespace-nowrap pointer-events-none">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div
                    class="lg:col-span-2 bg-[#111524] border border-slate-800/80 rounded-3xl overflow-hidden shadow-2xl flex flex-col">
                    <div
                        class="p-5 border-b border-slate-800/80 bg-[#161b2e] flex flex-col sm:flex-row items-center justify-between gap-3">
                        <div>
                            <h3 class="text-base font-extrabold text-white flex items-center gap-2">
                                <i class="fi fi-rr-box-alt text-cyan-400"></i>
                                Ürün Bazlı Satış Performansı
                            </h3>
                            <p class="text-xs text-slate-400 mt-0.5">Satılan adetler, elde edilen toplam gelir ve iptal
                                sayıları</p>
                        </div>

                        <input type="text" id="productReportSearch" onkeyup="filterReportProducts()"
                    ...
                    <div class="overflow-x-auto flex-1 max-h-96 custom-scrollbar">
                        <table id="productReportTable" class="w-full text-left text-xs text-slate-300">
                            <thead
                                class="bg-[#161b2e] text-slate-400 uppercase tracking-wider text-[10px] font-extrabold border-b border-slate-800 sticky top-0 z-10">
                                <tr>
                            ...
                            <tbody class="divide-y divide-slate-800/60 font-semibold">
                                @forelse($productStats as $stat)
                                    <tr class="hover:bg-slate-900/40 transition product-report-row">
                                        ...
                                        <td
                                            class="py-3.5 px-6 text-center {{ $stat->cancelled_qty > 0 ? 'text-rose-400 font-bold' : 'text-slate-600' }}">
                        ...
                        @forelse($categoryStatsMap as $cat)
                            ...
                            <div class="p-3 rounded-2xl bg-[#161b2e] border border-slate-800 space-y-2">
                                <div class="flex items-center justify-between text-xs">
                                    ...
                                    <span
                                        class="font-mono font-black text-emerald-400">₺{{ number_format($cat['total_revenue'], 2) }}</span>
                                </div>

                                <div class="w-full h-2 rounded-full bg-slate-900 overflow-hidden">
                                    <div class="h-full bg-gradient-to-r from-fuchsia-500 to-indigo-500 rounded-full"
                            ...
                            @forelse($waiterStats as $w)
                                ...
                                <tr class="hover:bg-slate-900/40 transition">
                                    <td class="py-4 px-6 font-extrabold text-white text-sm flex items-center gap-2">
                                        <div
                                            class="w-8 h-8 rounded-full bg-slate-800 text-indigo-300 font-bold flex items-center justify-center border border-slate-700">
    ...
    <div id="zReportModal"
        class="fixed inset-0 z-50 hidden flex items-center justify-center p-4 bg-slate-950/85 backdrop-blur-md no-print-bg">
        <div
            class="relative w-full max-w-md bg-[#111524] border border-slate-800 rounded-3xl p-6 shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div class="flex items-center justify-between pb-4 border-b border-slate-800 shrink-0">
                ...
                <button onclick="closeZReportModal()"
                    class="text-slate-400 hover:text-white p-1 rounded-lg hover:bg-slate-800 transition">✕</button>
            </div>

            <div class="flex-1 overflow-y-auto py-4 custom-scrollbar">
                <!-- 80mm POS TERMAL FİŞ BÖLGESİ -->
                <div id="zReportPrintArea"
                    class="bg-white text-slate-900 p-6 rounded-2xl shadow-inner font-mono text-xs border border-slate-200 mx-auto max-w-[320px] leading-relaxed">
                    ...
                    <div class="space-y-1 mb-3">
                        ...
                        <div class="flex justify-between"><span>💵 NAKİT TAHSİLAT</span><span
                                class="font-bold">₺{{ number_format($paymentBreakdown['nakit'], 2) }}</span></div>
                        <div class="flex justify-between"><span>💳 KREDİ KARTI</span><span
                                class="font-bold">₺{{ number_format($paymentBreakdown['kredi_karti'], 2) }}</span></div>
                        <div class="flex justify-between"><span>🎫 YEMEK KARTI</span><span
                                class="font-bold">₺{{ number_format($paymentBreakdown['yemek_karti'], 2) }}</span></div>
                        <div class="flex justify-between font-bold text-sm border-t border-slate-800 pt-1 mt-1"><span>TOPLAM
                                TAHSİLAT</span><span>₺{{ number_format($paymentBreakdown['total'], 2) }}</span></div>
                    </div>

                    <!-- CİRO VE FİNANS ÖZETİ -->
                    <div class="space-y-1 mb-3">
                        <div class="font-bold border-b border-slate-300 pb-1 mb-1">--- CİRO VE FİNANS ÖZETİ ---</div>
                        <div class="flex justify-between"><span>TOPLAM ADİSYON SAYISI</span><span
                                class="font-bold">{{ $stats['total_checks_count'] }} Adet</span></div>
                        <div class="flex justify-between"><span>ORTALAMA MASA TUTARI</span><span
                                class="font-bold">₺{{ number_format($stats['avg_check_amount'], 2) }}</span></div>
                        <div class="flex justify-between"><span>YAPILAN İSKONTO/İNDİRİM</span><span
                                class="font-bold">-₺{{ number_format($stats['total_discounts'], 2) }}</span></div>
                        <div class="flex justify-between"><span>İKRAM MALİYETİ ({{ $stats['complimentary_count'] }}
                                Adet)</span><span
                                class="font-bold">₺{{ number_format($stats['complimentary_total_amount'], 2) }}</span></div>
                        <div class="flex justify-between"><span>İPTAL/ZAYİ KAYBI ({{ $stats['cancelled_items_count'] }}
                                Adet)</span><span
                                class="font-bold">-₺{{ number_format($stats['cancelled_loss_amount'], 2) }}</span></div>
                        <div
                            class="flex justify-between font-extrabold text-sm border-t border-b border-dashed border-slate-800 py-1 my-1">
                            <span>NET CİRO</span><span>₺{{ number_format($stats['total_revenue'], 2) }}</span></div>
                    </div>
                    ...
                    <div class="space-y-1 mb-3">
                        ...
                        @foreach($categoryStatsMap as $cat)
                            <div class="flex justify-between"><span>{{ mb_strtoupper($cat['category_name']) }}
                                    ({{ $cat['sold_qty'] }})</span><span>₺{{ number_format($cat['total_revenue'], 2) }}</span>
                            </div>
                        @endforeach
                    </div>
                    ...
                    <div class="space-y-1 mb-3">
                        ...
                        @foreach($productStats->take(5) as $topProd)
                            <div class="flex justify-between"><span>{{ $topProd->sold_qty }}x
                                    {{ mb_strtoupper($topProd->product_name) }}</span><span>₺{{ number_format($topProd->total_revenue, 2) }}</span>
                            </div>
                        @endforeach
                    </div>
            ...
            <div class="pt-4 border-t border-slate-800 flex justify-between items-center shrink-0">
                <button onclick="closeZReportModal()"
                    class="px-4 py-2 rounded-xl text-slate-400 hover:bg-slate-800 text-xs font-bold transition">Kapat</button>
                <button onclick="printZReportSlip()"
                    class="px-6 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-black shadow-lg shadow-emerald-600/30 transition flex items-center gap-2 cursor-pointer">
