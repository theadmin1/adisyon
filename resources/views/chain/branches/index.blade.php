...
<div id="tableManager{{ $branch->id }}" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/80 p-4 backdrop-blur-sm" role="dialog" aria-modal="true">
    <div class="max-h-[94vh] w-full max-w-3xl overflow-y-auto rounded-2xl border border-slate-700 bg-slate-900 p-5 shadow-2xl sm:p-6">
        ...
        <div class="overflow-hidden rounded-xl border border-slate-800"><div class="max-h-80 overflow-auto"><table class="w-full min-w-[600px] text-left text-sm"><thead class="sticky top-0 bg-slate-950 text-[10px] uppercase text-slate-500"><tr><th class="p-3">Masa</th><th class="p-3">Salon</th><th class="p-3">Kapasite</th><th class="p-3">Durum</th>@if($canManage)<th class="p-3 text-right">İşlem</th>@endif</tr></thead><tbody class="divide-y divide-slate-800">@forelse($branch->diningTables as $table)<tr><td class="p-3"><strong>{{ $table->name }}</strong><small class="block font-mono text-slate-500">{{ $table->code?:'Kod yok' }}</small></td><td class="p-3 text-slate-400">{{ $table->hall?->name??'Salonsuz Alan' }}</td><td class="p-3">{{ $table->capacity }} kişi</td><td class="p-3"><span class="inline-flex items-center gap-1.5 text-xs {{ $table->is_active?'text-emerald-400':'text-slate-500' }}"><i class="h-1.5 w-1.5 rounded-full bg-current"></i>{{ $table->is_active?'Aktif':'Pasif' }}</span></td>@if($canManage)<td class="p-3"><div class="flex justify-end gap-2"><form method="POST" action="{{ route('chain.branches.tables.toggle',[$branch,$table]) }}">@csrf @method('PATCH')<button title="{{ $table->is_active?'Pasife al':'Aktifleştir' }}" class="rounded-lg border border-slate-700 p-2 {{ $table->is_active?'text-amber-400':'text-emerald-400' }}"><svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-width="1.8" d="M12 3v9m6.4-6.4a9 9 0 1 1-12.8 0"/></svg></button></form><form method="POST" action="{{ route('chain.branches.tables.destroy',[$branch,$table]) }}" onsubmit="return confirm('{{ $table->name }} silinsin mi?')">@csrf @method('DELETE')<button title="Sil" class="rounded-lg border border-rose-500/25 p-2 text-rose-400"><svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-width="1.8" d="M4 7h16m-10 4v6m4-6v6M9 7l1-3h4l1 3m-9 0 1 14h10l1-14"/></svg></button></form></div></td>@endif</tr>@empty<tr><td colspan="5" class="p-8 text-center text-slate-500">Bu şubede henüz masa yok.</td></tr>@endforelse</tbody></table></div></div>
    </div>
</div>
@endforeach
<script>
...
document.addEventListener('keydown',event=>{if(event.key==='Escape')document.querySelectorAll('[id^="tableManager"].flex').forEach(modal=>closeTableManager(modal.id.replace('tableManager','')))})
@if(str_starts_with((string)old('form_context'),'table_')) document.addEventListener('DOMContentLoaded',()=>openTableManager({{ (int)str_replace('table_','',(string)old('form_context')) }})); @endif
</script>
@endsection
