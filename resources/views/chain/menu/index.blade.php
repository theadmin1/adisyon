            ...
            @foreach($categories as $category)
                @foreach($category->products as $product)
                    ...
                    <tr data-menu-item data-product-id="{{ $product->id }}" data-category="{{ $category->id }}" data-status="{{ $productState }}" data-search="{{ $searchText }}" class="menu-product-row hover:bg-slate-800/30">
                        <td class="px-4 py-3"><div class="flex h-11 w-11 items-center justify-center overflow-hidden rounded-lg border border-slate-800 bg-slate-950">@if($product->image_path)<img src="{{ \Illuminate\Support\Str::startsWith($product->image_path,['http://','https://','data:']) ? $product->image_path : asset($product->image_path) }}" alt="" class="h-full w-full object-cover">@else<svg class="h-5 w-5 text-slate-700" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-width="1.5" d="m3 16 5-5 4 4 3-3 6 6M5 21h14V3H5v18Z"/></svg>@endif</div></td>
