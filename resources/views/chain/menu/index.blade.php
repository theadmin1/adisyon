...
<section class="overflow-hidden rounded-xl border border-slate-800 bg-slate-900">
    <div class="menu-toolbar border-b border-slate-800 p-4">
        <div class="grid gap-3 lg:grid-cols-[minmax(280px,1fr)_220px_190px_auto]">
            ...
            <select id="menuStatus" class="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2.5 text-sm"><option value="">Tüm durumlar</option><option value="published">Yayında</option><option value="pending">Yayın bekliyor</option><option value="unassigned">Şube atanmamış</option><option value="inactive">Pasif</option></select>
            ...
            @foreach($categories as $category)
                @foreach($category->products as $product)
                    @php
                        ...
                        $productState = !$product->is_active ? 'inactive' : ($assigned === 0 ? 'unassigned' : ($pending ? 'pending' : 'published'));
                    @endphp
    ...
    <div id="menuMobileRows" class="divide-y divide-slate-800 md:hidden">
        @foreach($categories as $category)@foreach($category->products as $product)
            @php($assigned=$product->branches->count()) @php($published=$product->branches->filter(fn($branch)=>(bool)$branch->pivot->published_at)->count()) @php($pending=$assigned-$published) @php($productState=!$product->is_active?'inactive':($assigned===0?'unassigned':($pending?'pending':'published')))
